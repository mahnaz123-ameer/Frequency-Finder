...
const AudioRecorder = () => {
    ...
    const createWaveSurfer = () => {
      ...
      record.on('record-end', (blob) => {
        ...
        recordedWaveSurfer.on('ready', () => {
          // Set up the analyser when the recording is ready
          ...
          fetch(recordedUrl)
            ...
            .then((data) => {
              audioContext.decodeAudioData(data, (buffer) => {
                ...
                // Create an analyser node and connect it
        ...
        // Play button for the recorded audio
        const button = container.appendChild(document.createElement('button'));
        button.textContent = 'Play';
        button.onclick = () => {
          recordedWaveSurfer.playPause();
          // Start frequency analysis when the play button is clicked
        ...
        recordedWaveSurfer.on('play', () => {
          ...
          // Start frequency analysis when the audio starts playing
          performFFTAnalysis();
        });
  ...
  const startRecording = async (deviceId) => {
    ...
    // Create an analyser node and connect to the stream
    analyserRef.current = audioContext.createAnalyser();
    source.connect(analyserRef.current);
  ...
  const performFFTAnalysis = () => {
    ...
    // Continuously update frequency every 100ms
